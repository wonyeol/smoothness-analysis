"""
Based on: `whitebox/refact/test/pyro_example/air_simplified.py`.
...
def main(data):
    ...
    with pyro.plate("data", 60000) as ix:
    #@@@@@
        # size = [64, 50, 50]
        batch = data[ix]
        ...
        for t in range(3):
            ...
            bl_value = torch.squeeze(bl_value, -1)
            infer_dict = dict(baseline=dict(baseline_value=bl_value))
            #===== baseline_step
            ...
            pyro.sample("aux_{}".format(t), Normal(p, 1.), obs=0.)
            # ================
