...
let buf_to_format (f: Buffer.t -> 'a -> unit)
    (fmt: form) (x: 'a): unit =
  let buf = Buffer.create 1 in
  f buf x;
  F.fprintf fmt "%s" (Buffer.contents buf)
...
let buf_string (buf: Buffer.t) (s: string): unit = Printf.bprintf buf "\"%s\"" s
...
let ss_fp = buf_to_format buf_ss
...
(** PP functions *)
let pp_bool chan b =
  Printf.fprintf chan "%b" b
let fp_bool fmt b =
  F.fprintf fmt "%b" b
let pp_pair pp1 pp2 chan (a,b) =
  Printf.fprintf chan "(%a, %a)" pp1 a pp2 b
let fp_pair fp1 fp2 fmt (a,b) =
  F.fprintf fmt "(%a, %a)" fp1 a fp2 b
let pp_list pp chan = function
  | [] -> Printf.fprintf chan "[ ]"
  | [x] -> Printf.fprintf chan "[ %a ]" pp x
  | x::l ->
      Printf.fprintf chan "[ %a" pp x;
      List.iter (Printf.fprintf chan ", %a" pp) l;
      Printf.fprintf chan " ]"
let fp_list fp fmt = function
  | [] -> F.fprintf fmt "[ ]"
  | [x] -> F.fprintf fmt "[ %a ]" fp x
  | x::l ->
      F.fprintf fmt "[ %a" fp x;
      List.iter (F.fprintf fmt ", %a" fp) l;
      F.fprintf fmt " ]"
