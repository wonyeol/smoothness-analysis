 ...
 **
 ** entry_support.ml: entry file for the command line support analysis *)
open Analysis_sig
...
let main () =
  ...
  | None, None ->
      ...
      let b =
        Ai_support.start_nr false { ao_do_num   = do_num;
                                    ao_debug_it = !debug;
                                    ao_input    = input;
                                    ao_wid_thr  = !do_thr;
  ...
  | Some m, Some g ->
      ...
      let b = Ai_support.start !debug [ aopts1; aopts2 ] [] [] in
      let s = if b then "VALID" else "POSSIBLY INVALID" in
