...
module Make = functor (D_num: ABST_DOMAIN_ZONE_NB) (D_typ: ABST_DOMAIN_NB_D) ->
  (struct
    ...
    let tensor_arange_get_type (t: t) (args: expr list): exp_ty =
      ...
      let ts =
        match IU.range_info_from_range args_iopt with
        | None           -> Some [None]
    ...
    let refine_dist_kind (args: expr list) (args_ty_l: exp_ty list)
      ...
      | _ -> failwith "refine_dist_kind: unimplemented case"
    ...
    let rec eval ac (t: t): t =
      ...
      | Sample (x, name, dist, args, Some obs, _) when not (can_nil t obs) ->
          eval (Assn(x,obs)) t
      | Sample (x, name, dist, args, obs_opt, _) ->
          ...
          let new_ac = Sample (x, name, new_dist, args, new_obs_opt, false) in
    ...
    let range_info e t = IU.range_info (get_exp_ty t e)
  end: ABST_DOMAIN_NB)
