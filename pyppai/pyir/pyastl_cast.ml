...
let to_expr_context (pyobj: Py.Object.t): expr_context =
  ...
  | "Del"   -> Del
  | "Param" -> Param
  | s  -> failwith (Printf.sprintf "to_expr_context: unreachable %S [%s]"
                      s (pyobj_str pyobj))
...
and to_expr (pyobj: Py.Object.t): 'a option expr =
  ...
  | "Constant" ->
      if false then Pyo.dump_obj (pyobj_attr pyobj "value");
      let ov = pyobj_attr pyobj "value" in
      let str = Py.Object.to_string ov in
      let disc =
        try
        ...
        with _ ->
          ...
          with _ ->
            match str with
            | "True" -> NameConstant (Some true, a)
            | "False" -> NameConstant (Some false, a)
            | _ -> Str (str, a) in
      disc
  | "Num"    ->
  ...
  | "Tuple" ->
      Tuple (pyobj_attr pyobj "elts"  |> to_list to_expr,
             pyobj_attr pyobj "ctx"   |> to_expr_context, a)
  | s -> failwith (Printf.sprintf "to_expr: unreachable %S [%s"
                   s (pyobj_str pyobj))
...
and to_arguments (pyobj: Py.Object.t): 'a option arguments =
  (pyobj_attr pyobj "args"        |> to_list to_arg,
   ...
   pyobj_attr pyobj "kwarg"       |> to_opt  to_arg,
