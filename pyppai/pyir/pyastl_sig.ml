 ...
 **
 ** pyastl_sig.ml: local abstract syntax tree for Python programs parsed by
 ...
 ** while this file  is for ast of Python *3.7* programs;
 ** there are slight differences between Python 3.7 and Python >=3.8.
 **
...
and 'a expr =
  ...
  | Subscript of 'a expr (* value *) * 'a slice (* slice *)
       * expr_context (* ctx *) * 'a
  | Starred of 'a expr (* value *) * expr_context (* ctx *) * 'a
...
(* ===== ignored in ir_cast.ml. ===== *)
and 'a arguments =
    'a arg list (* args *) * 'a arg option (* vararg *)
      * 'a arg list (* kwonlyargs *) * 'a expr list (* kw_defaults *)
      * 'a arg option (* kwarg *) * 'a expr list (* defaults *)
