...
module Pya = Pyastl_sig
module IU = Ir_util
module PU = Pyastl_util
...
let dist_kind_dict = [ (* adapted from `dist_kind_to_string_base` in ir_util.ml. *)
    (* continuous *)
    (Normal                , "Normal");
    ...
    (Dirichlet None        , "Dirichlet");
    (* discrete *)
    (Poisson               , "Poisson");
    (Categorical None      , "Categorical");
    (Bernoulli             , "Bernoulli");
    (OneHotCategorical None, "OneHotCategorical");
    (Delta                 , "Delta");
    (* subsample *)
    (Subsample (None,None) , "Subsample");
    (* new *)
    (LogNormal                   , "LogNormal");
...
let dist_kind_list: string list =
  List.map snd dist_kind_dict

let to_dist_kind (str: string): dist_kind =
  try fst (List.find (fun (_, s) -> s = str) dist_kind_dict) with
  | Not_found ->
     F.printf "unsupported dist_kind: %s\n" str;
     failwith "to_dist_kind: unimplemented case"
...
let _check field (e: 'a Pya.expr): bool * string =
  ...
  | `Deterministic ->
     (match e with
      | Pya.Attribute(Pya.Name("pyro", _, _), "deterministic", _, _) ->
         (true, "")
      | _ ->
         (false, ""))
  | `Factor ->
     (match e with
      | Pya.Attribute(Pya.Name("pyro", _, _), "factor", _, _) ->
         (true, "")
      | _ ->
         (false, ""))
  | `StrFormat ->
...
let rec to_expr ?(target: string option = None) :
  ...
  | Pya.Call (f, [rv_name; dist_expr], kwargs, _) when check `Sample f ->
      ...
      begin
        ...
        | [(Some "obs", obs)] ->
            ...
            let new_acmd =
              Sample(new_var, new_rv_name, new_dist_name,
                      new_dist_args, Some new_obs, false) in
            (acmd_list1 @ acmd_list2 @ acmd_list3 @ [new_acmd], Name(new_var))
        | _ ->
            failwith "to_expr: unimplemented case (Call: pyro.sample: other)"
      end
  | Pya.Call (f, [rv_name; val_expr], _, _) when check `Deterministic f ->
      (* pyro.deterministic("x", t) ===> pyro.sample("x", Delta(t)) (no observe) *)
      (* NOTE:
       * - Pyro implements pyro.deterministic by traslating it into pyro.sample with observe.
       * - But WL thinks it is still ok to use pyro.sample without observe.
       * - ref: https://docs.pyro.ai/en/dev/_modules/pyro/primitives.html#deterministic *)
      let (acmd_list1, new_rv_name) = to_expr rv_name in
      let (acmd_list2, new_val_expr) = to_expr val_expr in
      let new_var = get_name None in
      let new_acmd =
        Sample(new_var, new_rv_name, (Delta, []), [new_val_expr], None, false) in
      (acmd_list1 @ acmd_list2 @ [new_acmd], Name(new_var))
  | Pya.Call (f, [rv_name; logfactor_expr], _, _) when check `Factor f ->
      (* pyro.factor("x", t) ===> pyro.sample("x", Normal(t,1), obs=0) *)
      (* NOTE:
       * - This is not a semantic-preserving translation.
       * - But WL think it is still ok as it preserves the smoothness properties of a given program.
       * - ref: https://docs.pyro.ai/en/dev/_modules/pyro/primitives.html#factor *)
      let (acmd_list1, new_rv_name) = to_expr rv_name in
      let (acmd_list2, new_logfactor_expr) = to_expr logfactor_expr in
      let (zero_expr, one_expr) = (Num (Float 0.0), Num (Float 1.0)) in
      let new_var = get_name None in
      let new_acmd =
        Sample(new_var, new_rv_name, (Normal, []),
               [new_logfactor_expr; one_expr], Some zero_expr, false) in
      (acmd_list1 @ acmd_list2 @ [new_acmd], Name(new_var))
  ...
  | Pya.Call (f, args, kwargs, _) as e ->
      ...
      let new_f =
        ...
        | Pya.Attribute(Pya.Attribute(Pya.Attribute(Pya.Name(obj_name, _, _),
            ...
            Name (obj_name ^ "." ^ method_name1 ^ "." ^ method_name2 ^ "."
                  ^ method_name3)
        | _ ->
            F.printf "unsupported expression:\n%s" (Pyastl_dump.dump_expr e);
      ...
      let acmd_list3 =
        ...
        | Name "pyro.plate" ->
           ...
           let acmd1 =
             match new_args with
             | ((Str _) as arg0) :: args_rest when (List.length args_rest >= 2) ->
                 (* `__@@_indices(new_var) = Sample(name, Subsample(None, None), args_rest)' *)
                 let x = NameManager.plate_indices_name new_var in
                 let dist = (Subsample (None, None),  []) in
                 Sample (x, arg0, dist, args_rest, None, false)
             | (Str _) :: _ ->
                 (* if `size' and `subsize' are not given to the plate, do:
  ...
  | Pya.Attribute (e1, attr, _, _) ->
      ...
      let new_var = get_name target in
      let new_acmd =
        match new_e1, attr with
        | Name("constraints"), "positive" ->
            AssnCall(new_var, Name("RYLY[constraints.positive]"), [], [])
        | Name("constraints"), "negative" ->
            AssnCall(new_var, Name("RYLY[constraints.negative]"), [], [])
        | _ ->
            AssnCall(new_var, Name("RYLY"), [], []) in
  ...
  | Pya.Subscript (e1, e2, _, _) ->
      ...
      (acmd_list1 @ acmd_list2 @ [new_acmd], Name(new_var))
  | Pya.Starred (e, _, _) ->
      (* AssnCall: acmd -- "apply_star" *)
      let (acmd_list, new_e) = to_expr e in
      let new_var = get_name target in
      let new_acmd = AssnCall (new_var, Name("apply_star"), [new_e], []) in
      (acmd_list @ [new_acmd], Name(new_var))
  | Pya.List (elts, _, _)
  | Pya.Tuple (elts, _, _) ->
...
and to_dist_expr
  ...
  | Pya.Call(Pya.Name(dist_kind, _, _), dist_args, dist_kwargs, _) ->
      let new_dist_kind = to_dist_kind dist_kind in
      let new_dist = (new_dist_kind, dist_trans_l) in
      let (acmd_list1, new_dist_args) = to_expr_list dist_args in
      let (acmd_list2, new_dist_kwargs) = to_keyword_list dist_kwargs in
      (* see ai_diff.ml:diff_dk_sig for details. *)
      let kwargs_to_translate = ["logits"; "gate_logits"] in
      (* choose only those kwargs with keyword in `kwargs_to_translate`. *)
      let new_dist_args_from_kwargs =
        List.filter_map (function
            | Some arg_str, value
                 when List.mem arg_str kwargs_to_translate -> Some value
            | _ -> None)
          new_dist_kwargs in
      (acmd_list1 @ acmd_list2, new_dist, new_dist_args @ new_dist_args_from_kwargs)
  ...
  | dist_expr ->
     F.printf "unsupported expression: %s\n" (Pyastl_dump.dump_expr dist_expr);
...
let rec _stmt_to_prog
  ...
  | Pya.FunctionDef (id, _, _, _, _, _) ->
      (* WL: ignore translating a function def. *)
      (* failwith "_stmt_to_prog: unimplemented case (FunctionDef)" *)
      Format.printf "_stmt_to_prog(ir_cast.ml): ignore translating a FunctionDef `%s`.\n" id;
      [ skip_prog ]
  | Pya.Return (None, _) ->
      [ skip_prog ]
  ...
  | Pya.AugAssign (target, op, value, a) ->
     _stmt_to_prog final_prog_list (Pya.Assign ([target], Pya.BinOp(target, op, value, a), a))
  ...
  | Pya.While (test, body, [], _) ->
      (* XR: I do not understand this code;
       *     also, it seems incorrect to me, as it unrolls the loop once,
       *     and adds a strange assertion at the head of the loop *)
